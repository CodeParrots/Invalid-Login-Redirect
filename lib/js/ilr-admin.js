...
( function( $ ) {
	...
	var roleRedirects = {
		...
		applyToAll: function( e ) {
			...
			var $containers    = $( '.role-redirects.add-on' ).find( '.ilr-notice' ).not( ':nth-child(2)' );
			    $clicked       = $( e.currentTarget ),
					$parent        = $clicked.parents( '.fields' ),
			    valid_login    = $parent.find( 'input.valid-login' ).val(),
			    invalid_login  = $parent.find( 'input.invalid-login' ).val();
